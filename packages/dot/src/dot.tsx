...
export const Dot = React.forwardRef<HTMLSpanElement, DotProps>((props, ref) => {
  const { color, variant = 'solid', className, size = '2xs', bordered = false, ...rest } = props;

  const classes = useDotClass({ variant, bordered, size });
  return <span ref={ref} className={cx(classes, className)} data-color={color ? color : undefined} {...rest} />;
});
