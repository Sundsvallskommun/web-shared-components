...
export type TextareaProps<T = HTMLTextAreaElement> = IInputProps<T> &
  TextareaHTMLAttributes<T> & ITextareaProps &
  ...
  (props, ref) => {
    ...
    return (
      <div>
        ...
        />
        { maxLengthWarning &&
            <p className="sr-only" aria-live="assertive" role="alert">{maxLengthWarningText}</p>
        }
        { showCount &&
            <div className="form-textarea-counter">{charCount}/{maxLength}</div>
        }
      </div>
    );
  }
