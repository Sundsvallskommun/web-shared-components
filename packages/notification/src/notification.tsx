...
import { Icon, InfoIcon, XIcon, XCricleIcon, CheckCircleIcon, ExclamationIcon } from '@sk-web-gui/icon';
  ...
  return (
    <div ref={ref} className={clsx('notification', className)}>
      ...
      <div className="notification-action">
        ...
        {closeable && (
          <button onClick={onClose} className="notification-action-close-button">
            {customCloseIcon ? (
              (customCloseIcon as React.ReactNode)
            ) : (
              <Icon as={XIcon} label="x" className="text-current align-text-bottom h-3.5 w-3.5" />
            )}
          </button>
        )}
      </div>
