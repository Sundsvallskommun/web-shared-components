...
export interface DraggableOptionsType {
  showMovedAwayOnDrag?: boolean;
}
...
export const Draggable = class {
  ...
  handleDragEnter = (e: MouseEvent) => {
    ...
    if (closestMenuItem !== null) {
      if (!closestMenuItem.className.includes('moved-away')) {
  ...
  handleDragStart = (e: MouseEvent) => {
    ...
    if (closestMenuItem) {
      ...
      if (this.options?.showMovedAwayOnDrag) {
        this.draggedElement.classList.add('moved-away');
      } else {
        this.draggedElement.classList.add('moving');
      }
  ...
  handleDragEnd = (e: MouseEvent) => {
    if (!this.dragging) return;
    this.handleDrop(e);
    this.draggedElement && this.draggedElement.classList.remove('moved-away');
    this.draggedElement && this.draggedElement.classList.remove('moving');
  ...
  handleDrop = (e: MouseEvent) => {
    ...
    if (closestMenuItem?.className.includes('moved-away')) return;
    if (closestMenuItem?.className.includes('dragenter')) {
      ...
      if (
        ...
        // this.draggedElement && this.draggedElement.classList.add('moved-away');
  ...
  createGhostElement = (draggedElement: Element) => {
    ...
    this.draggedElementGhost = draggedElement.cloneNode(true);
    this.draggedElementGhost.style.position = 'fixed';
    const draggedElementGhostBoundingClientRect = draggedElement.getBoundingClientRect();
